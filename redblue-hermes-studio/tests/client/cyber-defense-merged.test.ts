...
describe('merged RedBlue Hermes studio', () => {
  ...
  it('uses a cancellable live stream for task conversations', () => {
    ...
    expect(source).not.toContain(':disabled="chatLoading"')
    expect(source).toContain("emit('run-state'")
    expect(source).toContain('history-run-banner')
    expect(source).toContain('runningTaskId !== selectedTaskId')
    expect(source).toContain('if (selectedTaskId.value === taskId) {')
    expect(source).toContain('chatLoading && runningTaskId === selectedTaskId')
    expect(source).toContain("runTerminalStatus === 'completed'")
    expect(source).toContain("await completeTasks([taskId], '智能体问答已完成")
    expect(source).toContain('taskDisplayStatus(task)')
    expect(source).not.toContain('await runCyberDefenseChat')
    expect(view).toContain("v-if=\"cachedViews.has('tasks')\"")
    expect(view).toContain('v-show="activeView === \'tasks\'" embedded')
    ...
    expect(view).toContain('@task-select="selectTask"')
    expect(view).toContain('@run-state="handleTaskRunState"')
    expect(view).toContain('top-run-indicator')
    expect(view).toContain('.slice(-40)')
  })
