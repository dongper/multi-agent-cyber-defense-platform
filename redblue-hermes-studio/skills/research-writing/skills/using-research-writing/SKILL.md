...
## 长任务定期反馈（user preference）

**User has complained multiple sessions: "做成啥样了，你要不定期给我一些反馈的" / "开始呀" / "写怎么样了" when the agent goes silent during long tasks.** They expect progress reports every 3–5 minutes during multi-step work, not a single silent run.

When a writing task is estimated to take more than 5 minutes (long experiment runs, multiple chapters, multi-stage review, dataset downloads):

1. **First feedback within 2 minutes** — even if just "正在做 X, 5 分钟后报告". Do NOT let the conversation go silent after kicking off a long process.
2. **Update every 3–5 minutes** — at minimum, one short message per checkpoint: "完成 A, 开始 B", "实验跑到 50%, 预期 3 分钟", "X 阶段遇到 Y 问题, 正在修复".
3. **If blocked, say so immediately** — don't silently retry or pretend things are fine. State the blocker in one sentence and ask for input.
4. **Use a progress table**:
   | 任务 | 状态 |
   | 数据下载 | ✅ |
   | 实验代码 | 🔄 进行中 |
   | 写论文 | ⏸ 等待 |
5. **Do NOT auto-retry failed network/API calls** — if a curl/API call times out, STOP and ask the user before retrying. Caught in session 2026-06-28: I retried a CrossRef query after a timeout, the system blocked it ("stop, do NOT rephrase, do NOT attempt the same outcome via a different command"). The correct response is: stop, summarize state, ask the user how to proceed.
