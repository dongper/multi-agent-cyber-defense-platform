...
| 网络/API 调用成功 | 看到 HTTP 2xx 响应 + 解析后的数据 | "应该成功了" / 命令超时未确认 |
| 子代理任务完成 | 看到子代理返回的具体证据 (URL/ID/路径/状态码) | 接受子代理"我做了"叙述而不验证 |
...
- 想着"就这一次"
- 疲劳想结束工作
- **网络/API 命令超时/被取消后直接重试** — timeout 意味着状态未知，盲目重试可能做错。停止，问用户。
- **子代理返回"我做了 X"但没给具体证据（URL/ID/路径/HTTP 状态码）** — 必须独立验证或拒绝信任。
- **子代理声称"我没有 web 工具"但仍给出基于训练数据的"答案"** — 这是最危险的模式：把猜测包装成答案。绝对不能信任。直接抛弃输出，自己用实际工具验证。
