...
**Running multiple bots of the same platform (e.g. two WeCom bots):** use Profiles — each profile gets an independent gateway. See `references/multi-bot-wecom-setup.md` for the full walkthrough and pitfalls (Weixin conflict on clone, error 853000, etc.).
...
To run two bots of the same platform (e.g. two WeCom or two Telegram bots), create a cloned Profile with its own credentials. See `references/multi-bot-same-platform.md` for the full pattern (clone → edit .env → start gateway).
...
**`.env` file is protected from `patch`/`write_file` tool writes.**
Hermes blocks tool-based writes to `~/.hermes/.env` to prevent
credential overwrites. To add or modify env vars (new provider keys,
base URL overrides, etc.), use the terminal instead:
...
Alternatively, `hermes config set` handles `config.yaml` entries directly
and does NOT require .env editing (for settings that live in config.yaml
rather than .env).
...
### Gateway connectivity issues (platform-specific)
Each messaging platform connects via its own WebSocket/HTTP endpoint. If a platform shows as disconnected or messages aren't flowing, check the error log:

```bash
grep -i "weixin\|wecom\|error\|disconnect" ~/.hermes/logs/gateway.error.log | tail -30
```

Common platform endpoint errors:

| Platform | Expected endpoint | Typical error |
|----------|-------------------|---------------|
| Weixin (微信) | `ilinkai.weixin.qq.com:443` | `Cannot connect to host ilinkai.weixin.qq.com:443 ssl:default [nodename nor servname provided, or not known]` |
| WeCom (企业微信) | `openws.work.weixin.qq.com:443` | `WebSocket error: WeCom websocket closed` / `Cannot connect to host openws.work.weixin.qq.com:443` |

**Diagnostic pattern:**
1. First test DNS from terminal: `nslookup ilinkai.weixin.qq.com` or `curl -sI https://ilinkai.weixin.qq.com`. If terminal DNS works but gateway logs show `nodename nor servname provided`, the problem is **async resolver state**, not real DNS failure.
2. **Fix:** Restart the gateway (`hermes gateway restart`). This forces re-resolution of all platform hostnames.
3. If DNS genuinely fails from terminal, check network interface status, VPN/proxy settings, and system DNS config (`scutil --dns` on macOS).

### Gateway issues (general)
Check logs first:
```bash
...
Hermes Web UI is the official web dashboard for Hermes Agent — a full-featured desktop app and web interface for managing AI chat, sessions, platforms, models, skills, and more.
...
**Pitfall (China/npm timeout):** Default npm registry may timeout. Use Chinese mirror first:
```bash
...
- **Data home:** `~/.hermes-web-ui/`
- **Server log:** `~/.hermes-web-ui/server.log`
- **PID file:** `~/.hermes-web-ui/server.pid`
- **Auth token:** Auto-generated on first startup, shown in startup logs

### Features

- **AI Chat** — Real-time streaming via Socket.IO, multi-session management, Markdown rendering
- **Platform Channels** — Configure 8 platforms (Telegram, Discord, Slack, WhatsApp, Matrix, Feishu, WeChat, WeCom)
- **Usage Analytics** — Token usage, session counts, cost estimation, model distribution charts
- **Scheduled Jobs** — Create, edit, pause, resume, delete cron jobs
- **Model Management** — Auto-discover models, manage providers, OAuth login
- **Skills & Memory** — Browse, install skills; manage memory
- **Web Terminal** — Integrated terminal via node-pty + xterm.js
- **Profiles** — Switch between Hermes profiles
...
| `HERMES_WEB_UI_HOME` | `~/.hermes-web-ui` | Web UI data home |
| `AUTH_TOKEN` | auto-generated | Explicit bearer token |
| `HERMES_HOME` | `~/.hermes` | Hermes data home |
| `HERMES_BIN` | `hermes` | Custom Hermes CLI binary path |

### Desktop App (Hermes Studio)

The Electron desktop app is branded **Hermes Studio** (not "hermes-web-ui"). Download from [GitHub Releases](https://github.com/EKKOLearnAI/hermes-web-ui/releases/latest) — available for macOS (arm64 DMG), Windows (.exe), Linux (.deb, .AppImage).
...
# Mount + install — use `open`, NOT `hdiutil attach` (see pitfall below)
open ~/Downloads/Hermes.Studio-0.6.9-arm64.dmg
sleep 3  # wait for mount

# Copy to Applications
cp -R "/Volumes/Hermes Studio 0.6.9-arm64/Hermes Studio.app" /Applications/

# Unmount
hdiutil detach "/Volumes/Hermes Studio 0.6.9-arm64"

# Launch
open -a "Hermes Studio"
```

**Pitfall (`hdiutil attach` fails):** On macOS, `hdiutil attach` can fail with "资源暂时不可用" (resource temporarily unavailable), especially after a browser download or if the DMG was recently accessed. Use `open` instead — it reliably mounts via Finder's DMG handler. Wait 2-3 seconds after `open` before accessing `/Volumes/...`.

**Key difference from npm version:** The npm `hermes-web-ui` installs a **web server** (accessed via browser at localhost:8648). The desktop app is a standalone **Electron app** (Hermes Studio) with native tray integration, auto-updates, and its own window. Both connect to the same Hermes Agent backend.
...
Hermes Web UI is the official desktop app and web dashboard for Hermes Agent. It provides a GUI for chat, session management, platform channel configuration, usage analytics, cron jobs, model management, skills browsing, and an integrated web terminal.
...
# In China or slow networks, use npmmirror registry
npm config set registry https://registry.npmmirror.com && npm install -g hermes-web-ui

# Start (daemon mode, opens http://localhost:8648)
hermes-web-ui start
...
# Download .dmg from GitHub releases (use API to find URL)
curl -s https://api.github.com/repos/EKKOLearnAI/hermes-web-ui/releases/latest | grep '"browser_download_url".*\.dmg"'

# Example for arm64:
curl -L -O https://github.com/EKKOLearnAI/hermes-web-ui/releases/download/vX.Y.Z/Hermes.Studio-X.Y.Z-arm64.dmg

# Mount, copy to Applications, unmount
hdiutil attach Hermes.Studio-*.dmg
cp -R "/Volumes/Hermes Studio */Hermes Studio.app" /Applications/
hdiutil detach "/Volumes/Hermes Studio "*

# Launch
open -a "Hermes Studio"
```

On first launch, the desktop app prompts to download a runtime — choose **Cloudflare** (faster CDN) or GitHub. This downloads the Hermes Agent runtime (Python env + dependencies). The app uses the native `~/.hermes` data directory, so existing CLI config/sessions/skills carry over.

### Pitfalls

- **npm install hangs/times out in China** — set registry to `https://registry.npmmirror.com` first.
- **Desktop app conflicts with npm-based server** — stop the npm server (`hermes-web-ui stop`) before launching the desktop app, or vice versa.
- **Desktop app uses `~/.hermes`** — shares data with CLI. Profiles, sessions, skills, and gateway configs are all shared.
- **arm64 vs x64** — always check `uname -m` before downloading. Apple Silicon Macs (M1/M2/M3/M4) use arm64; older Intel Macs use x64.

See `references/hermes-web-ui-macos-setup.md` for detailed macOS installation walkthrough.
